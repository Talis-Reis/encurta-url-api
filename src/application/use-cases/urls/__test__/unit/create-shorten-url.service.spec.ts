...
describe('CreateShortenUrlService Unit Test', () => {
    ...
    beforeEach(async () => {
        ...
        shortenUrlRepository = module.get<IShortenUrlRepository>(IShortenUrlRepository)
    ...
    it('should generate a unique short code and return the shortened URL', async () => {
        // Simula que o código nunca existe (primeira tentativa já é única)
        jest.spyOn(shortenUrlRepository, 'getByShortCode').mockResolvedValueOnce(undefined as unknown as Urls)
        jest.spyOn(shortenUrlRepository, 'createShortenUrl').mockResolvedValueOnce(undefined)
        jest.spyOn(envConfig, 'getAppDomain').mockReturnValue('https://encurta.dev')
        ...
        // Força o código gerado para ser previsível
        jest.spyOn<any, any>(service, 'generateShortCode').mockReturnValue('abc123')

        const result = await service.execute('https://original.com', 42)

        expect(shortenUrlRepository.getByShortCode).toHaveBeenCalledWith('abc123')
        ...
        expect(getUrlDomainMock).toHaveBeenCalledWith('https://encurta.dev', 3000, 'abc123')
        expect(result).toBe('https://encurta.dev/abc123')
    })

    it('should retry if short code already exists', async () => {
        // Primeira tentativa: código existe, segunda: não existe
        ...
        jest.spyOn(shortenUrlRepository, 'getByShortCode')
            .mockResolvedValueOnce({} as Urls) // existe
            .mockResolvedValueOnce(undefined as unknown as Urls) // não existe
        jest.spyOn(shortenUrlRepository, 'createShortenUrl').mockResolvedValueOnce(undefined)
        jest.spyOn(envConfig, 'getAppDomain').mockReturnValue('https://encurta.dev')
        jest.spyOn(envConfig, 'getAppPort').mockReturnValue(3000)
        const getUrlDomainMock = getUrlDomain as jest.Mock
        ...
        expect(shortenUrlRepository.getByShortCode).toHaveBeenNthCalledWith(1, 'exist1')
        expect(shortenUrlRepository.getByShortCode).toHaveBeenNthCalledWith(2, 'uniq12')
        expect(shortenUrlRepository.createShortenUrl).toHaveBeenCalledWith(
            'https://original.com',
            'uniq12',
            undefined,
        ...
        expect(envConfig.getAppDomain).toHaveBeenCalled()
        expect(envConfig.getAppPort).toHaveBeenCalled()
        expect(getUrlDomainMock).toHaveBeenCalledWith('https://encurta.dev', 3000, 'uniq12')
